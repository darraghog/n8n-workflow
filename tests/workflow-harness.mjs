...
function runBuildResultCode(jsCode, inputJsonItems) {
  const context = {
    $input: {
...
function testStructure(workflow) {
  ...
  assert.equal(c["Build Result"].main[0][0].node, "Send Email");
}
...
function testBuildResultCodeNode(workflow) {
  ...
  const out1 = runBuildResultCode(jsCode, [form, ollama]);
  ...
  const out1b = runBuildResultCode(jsCode, [ollama, form]);
  ...
  const out2 = runBuildResultCode(jsCode, [
  ...
  const out3 = runBuildResultCode(jsCode, [
    {
